...
export class CookieStore {
  ...
  constructor(cookies: Record<string, string>, options?: CookieStoreOptions) {
    this.options = options
    this.input = cookies
    this.output = []

    this.encode = options?.encode ?? ((value) => value)
    this.decode = options?.decode ?? ((value) => value)
  }
  ...
  async get(name: string): Promise<any> {
    ...
    if (value) {
      return await this.decode(value)
    }
  }

  async set(name: string, value: unknown, options?: CookieSerializeOptions): Promise<void> {
    this.output.push({
      name,
      value: await this.encode(value),
      options: { ...this.options?.serialize, ...options },
    })
  }
}
